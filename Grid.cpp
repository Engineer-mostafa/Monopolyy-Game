...
{
	if (pos.IsValidCell() && CellList[pos.VCell()][pos.HCell()]->GetGameObject() != NULL) // Check if valid position
	{
	...
	for (int i = NumVerticalCells - 1; i >= 0; i--) {
		for (int j = 0; j < NumHorizontalCells; j++)
		{
			if ((CellList[i][j])->HasLadder()) {
				CellList[i][j]->GetGameObject()->Save(OutFile, L, type);
				L++;
			}
			else if ((CellList[i][j])->HasSnake()) {
				CellList[i][j]->GetGameObject()->Save(OutFile, S, type);
				S++;
			}
			else if ((CellList[i][j])->HasCard()) {
				CellList[i][j]->GetGameObject()->Save(OutFile, C, type);
				C++;
			}
		}
	}
