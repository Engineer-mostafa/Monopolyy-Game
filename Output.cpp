...
void Output::CreateDesignModeToolBar() const
{
	...
	MenuItemImages[ITM_Delete_Game_Object] = "images\\Menu_Killer.jpg";
...
void Output::DrawCell(const CellPosition & cellPos, int cardNum) const
{
	...
	pWind->DrawRectangle(cellStartX, cellStartY, cellStartX + 110, cellStartY + 55, FILLED, 1, 1);
...
void Output::DrawPlayer(const CellPosition & cellPos, int playerNum, color playerColor) const
{
	...
	if (playerNum < 0 || playerNum > 3 || cellPos.VCell() > 8 || cellPos.HCell() > 10)
		return;
...
void Output::DrawLadder(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	for (int i = toStartY; i < fromStartY; i = i + 55) {
		pWind->DrawLine(x12, i + 55, x34, i + 55);
	}
...
void Output::DrawSnake(const CellPosition & fromCell, const CellPosition & toCell) const
{
	...
	else if ((fromCell.GetCellNum() - toCell.GetCellNum())>0 && (fromCell.GetCellNum() - toCell.GetCellNum()) % 11 == 0)
	{
		...
		int x[4] = { x12,+x12 - (xChange),x12,x12 + (xChange) };
		int y[4] = { y1,y1 + (yChange),y1 + 2 * yChange,y1 + (yChange) };
